...
exports.login = async (req, res) => {
  ...
  try {
    ...
    let user = await db.execute(db.partsku, query, data.email)
    if(user.length > 0) {
      // validate pass
      if(encryption.validatePass(data.password, user[0].pass)) {
        ...
        sqlData.id = user.id
        sqlData.name = user.name
        sqlData.sid = user.sid;
        ...
        res.json(sqlData)
      } else res.status(400).json('Email / Password is not valid!')
    } else res.status(404).json('Email / Password is not valid!')
  } catch (e) {
    // console.log(e)
    res.status(400).json(e)
  }
